...
export default function() {
  ...
  function render(max) {
    let n = series[0].data.length;

    chart.x().domain([0, max]);
    chart.y().domain(d3.range(n));
    chart.y_axis().tickValues(d3.range(3, n, 3));
    poles.datum((d,i) => series[i]).call(chart);
  }

  function component(svg) {
  ...
  component.run = function(run, timestep, counter) {
    let dragonfly = model.dragonfly;
